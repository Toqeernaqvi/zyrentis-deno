...
See the
[official getting started guide](https://neon.com/docs/get-started/signing-up.md)
...
- **1 project**: ask "Would you like to use '{project_name}' or create a new
  one?"
- **Multiple projects (<6)**: list all and let them choose
- **Many projects (6+)**: list recent projects, offer to create new or specify
  by name/ID
...
3. If unreadable (permissions): use append command or show the line to add
   manually
4. Never overwrite an existing `.env` — always append or update in place

### 3. Install Driver

Choose based on deployment platform. For detailed guidance, see
...
For serverless driver patterns, see `neon-serverless.md`. For complex scenarios
(multiple runtimes, hybrid architectures), see `connection-methods.md`.

### 4. Authentication (if needed)

Skip for CLI tools, scripts, or apps without user accounts.

If the app needs auth: use MCP server `provision_neon_auth` tool, then see
`neon-auth.md` for setup. For auth + database queries, see `neon-js.md`.

### 5. ORM Setup (optional)

Check for existing ORM (Prisma, Drizzle, TypeORM). If none, ask if they want
one. For Drizzle integration, see `neon-drizzle.md`.
...
Installs the VSCode extension and configures the MCP server. See `devtools.md`
for details.
...
- Neon-specific features (branching, autoscaling, scale-to-zero) — see
  `features.md`
