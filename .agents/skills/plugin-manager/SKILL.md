...
Maintain plugin packaging for both ecosystems in a single repository, using a
shared `plugins/` directory and a shared top-level `skills/` source of truth.

## Working Rules

- Keep each plugin self-contained in `plugins/<plugin-name>/`.
- Keep shared reusable skill content in top-level `skills/`.
- Expose shared skills per plugin via symlinks in
...
- **Marketplace manifests**
  ...
  - `.cursor-plugin/marketplace.json` uses `metadata.pluginRoot: "plugins"` and
    plugin entries with `source: "<plugin-name>"`.
