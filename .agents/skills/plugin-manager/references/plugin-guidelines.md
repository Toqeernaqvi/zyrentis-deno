...
- Each plugin can own its own MCP configuration in
...
4. Add plugin to both marketplaces:
   ...
   - `.cursor-plugin/marketplace.json`: with `pluginRoot: "plugins"`, use
     `source: "<plugin-name>"`
...
- Keep plugin names kebab-case and consistent between marketplace and plugin
  manifests.
- Keep Cursor marketplace `pluginRoot` and `source` consistent to avoid
  double-prefix path resolution errors.
